...
def add_protected_repo(filename, repo_relative_path, repo_id):
    '''
    ...
    try:
        f = ProtectedRepoListingFile(filename)
        f.load()
...
def delete_protected_repo(filename, repo_relative_path):
    '''
...
def read_protected_repo_listings(filename):
    '''
