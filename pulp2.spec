...
mkdir -p %{buildroot}/%{_usr}/lib/gofer/plugins
mkdir -p %{buildroot}/%{_var}/lib/%{name}
mkdir -p %{buildroot}/%{_var}/lib/%{name}/uploads
mkdir -p %{buildroot}/%{_var}/lib/%{name}/repos
mkdir -p %{buildroot}/%{_var}/lib/%{name}/repos
mkdir -p %{buildroot}/%{_var}/lib/%{name}-cds
mkdir -p %{buildroot}/%{_var}/lib/%{name}-cds/packages
mkdir -p %{buildroot}/%{_var}/lib/%{name}-cds/packages
mkdir -p %{buildroot}/%{_var}/log/%{name}
mkdir -p %{buildroot}/%{_var}/log/%{name}-cds
mkdir -p %{buildroot}%{_datadir}/%{name}/selinux/server
...
# PKI
cp etc/pki/pulp/* %{buildroot}/etc/pki/%{name}
...
# CDS
cp etc/gofer/plugins/cdsplugin.conf %{buildroot}/%{_sysconfdir}/gofer/plugins
cp -R src/pulp/agent/gofer/cdsplugin.py %{buildroot}/%{_libdir}/gofer/plugins
cp etc/httpd/conf.d/pulp-cds.conf %{buildroot}/%{_sysconfdir}/httpd/conf.d/

# Init (init.d)
cp etc/rc.d/init.d/* %{buildroot}/%{_sysconfdir}/rc.d/init.d/
ln -s etc/rc.d/init.d/goferd %{buildroot}/%{_sysconfdir}/rc.d/init.d/pulp-agent

# SELinux Policy
%if %{pulp_selinux}
pushd selinux/server
./install.sh %{buildroot}%{_datadir}
mkdir -p %{buildroot}%{_datadir}/%{name}/selinux/server
cp enable.sh %{buildroot}%{_datadir}/%{name}/selinux/server
cp uninstall.sh %{buildroot}%{_datadir}/%{name}/selinux/server
cp relabel.sh %{buildroot}%{_datadir}/%{name}/selinux/server
popd
%endif
...
%config(noreplace) %{_sysconfdir}/%{name}/repo_auth.conf
...
%{python_sitelib}/%{name}/agent/handler/
