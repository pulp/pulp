...
from base import (assert_auth_CREATE, assert_auth_READ, assert_auth_UPDATE, assert_auth_DELETE,
                  assert_auth_EXECUTE)
from pulp.server.exceptions import OperationPostponed, MissingResource, InvalidValue, MissingValue
...
class TestconsumerGroupView(unittest.TestCase):
    ...
    def test_create_consumer_group(self, mock_factory, mock_resp, mock_iri_to_uri):
        ...
        request = mock.MagicMock()
        request.body_as_json = {'id': 'foo', 'display_name': 'bar'}
    ...
    def test_create_consumer_group_invalid_param(self):
        ...
        """
        request = mock.MagicMock()
        request.body_as_json = {'id': 'foo', 'display_name': 'bar', 'invalid_param': 'some'}
        consumer_group = ConsumerGroupView()
        try:
            response = consumer_group.post(request)
