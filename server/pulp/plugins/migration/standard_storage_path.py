...
class Plan(object):
    ...
    :ivar key_fields: List of fields that make up the unit key.
    :type key_fields: tuple
    :ivar is_file: Indicates the storage path references an individual file.
    :type is_file: bool
    """

    def __init__(self, collection, key_fields, is_file=True):
        """
        :param collection: A DB collection.
        :type collection: pymongo.collection.Collection
        :param key_fields: List of fields that make up the unit key.
        :type key_fields: tuple
        :ivar is_file: Indicates the storage path references an individual file.
        :type is_file: bool
        """
    ...
    def __iter__(self):
        ...
        fields = dict(
            (k, True) for k in chain(self.key_fields, base_fields)
        )
