...
`fixed in Pulp 2.4.1 <https://bugzilla.redhat.com/buglist.cgi?list_id=1242840&resolution=---&resolution=CURRENTRELEASE&classification=Community&target_release=2.4.1&query_format=advanced&bug_status=VERIFIED&bug_status=CLOSED&product=Pulp>`_.
...
connect to a 2.4.0 Pulp server or node. Be sure to pay attention to all new settings, for example
the new verify_ssl and ca_path settings.

Repeat this same process with the ``/etc/pulp/nodes.conf.rpmnew`` file, which has also introduced
