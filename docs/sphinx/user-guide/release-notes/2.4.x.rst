...
Every API that returns an HTTP 202 ACCEPTED response code has changed. For the sake of brevity, we
will not list every API that returns 202 here.
* The structure of the Call Report has been changed significantly.  It now returns a single
  ...
  Link to old and new CallReports:

  https://pulp-dev-guide.readthedocs.org/en/pulp-2.3/conventions/sync-v-async.html#call-report
  https://pulp-dev-guide.readthedocs.org/en/pulp-2.4/conventions/sync-v-async.html#call-report

``/v2/queued_calls/``

    This API has been removed in 2.4.
...
``/v2/tasks/<task_id>/``
    ...
    The data structure returned has changed.

    https://pulp-dev-guide.readthedocs.org/en/pulp-2.3/integration/rest-api/dispatch/task.html#polling-task-progress
    https://pulp-dev-guide.readthedocs.org/en/pulp-2.4/integration/rest-api/dispatch/task.html#task-report
...
TODO: Did searching for tasks change?
