...
#. Pulp has tightened the security in version 2.4.0 by adding functionality to validate server
   SSL cert against the trusted CA cert. More information about these certificates generated
   at the install time can be found in the :ref:`Installation Guide <ssl_validation>`.
...
Pulp 2.4.0 has an added functionality to validate server SSL cert against trusted CA certs.
To take advantage of it, you can run ``pulp-gen-ca-certificate`` to generate a CA cert, SSL cert
...
:ref:`Installation Guide <ssl_validation>`. After generating these certificates and restarting apache,
you will need to copy and add the CA cert to the trusted CA certs on the consumers and hosts
running pulp-admin install. For pulp-admin, you can follow instructions located at
:ref:`Installation Guide <admin_trusted_ca_installtion>` and for consumers, you can refer to
:ref:`Installation Guide <consumer_trusted_ca_installtion>`.
