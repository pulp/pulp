...
For systemd systems::
    ...
    $ sudo systemctl restart pulp-agent
...
latest RPMs from the Pulp repository and run the database migrations::
    ...
    $ sudo yum groupupdate pulp-server
...
After all Pulp servers and nodes have been upgrade, all consumers need to be re-registered. On
