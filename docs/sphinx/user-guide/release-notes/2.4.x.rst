...
To upgrade to the new Pulp release from version 2.3.x, your message broker needs to be running
and configured. Perform the upgrade using yum to install the latest RPMs from the Pulp repository
and run the database migrations as the ``apache`` user:

::

    $ sudo yum groupupdate pulp-server-qpid
