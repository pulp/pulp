...
def add_and_remove(repository_pk, add_content_units, remove_content_units):
    ...
    with transaction.atomic():
        ...
        new_version = models.RepositoryVersion(repository=repository)
        new_version.number = repository.last_version + 1
        new_version.save()
        created = models.CreatedResource(content_object=new_version)
        created.save()
        repository.last_version = new_version.number
        repository.save()
    ...
    try:
        ...
        with transaction.atomic():
            new_version.delete_incomplete_version()
            created.delete()
        raise
