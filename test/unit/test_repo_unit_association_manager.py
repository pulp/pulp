...
class RepoUnitAssociationManagerTests(testutil.PulpTest):
    ...
    def test_unassociate_by_key(self):
        """
        Tests removing an association by its key that exists.
        """
        pass

    def test_unassociate_by_key_no_association(self):
        """
        Tests unassociating a unit where no association exists.
        """
        pass

    def test_unit_associations_for_repo(self):
        """
        Tests retrieving associations for a repo only returns associations for
        the requested repo.
        """
        ...
        self.manager.associate_unit_by_id('repo-2', 'type-1', 'unit-a')
        self.manager.associate_unit_by_id('repo-2', 'type-2', 'unit-b')

        # Test
        associations = self.manager.unit_associations_for_repo('repo-1')

        # Verify
        self.assertEqual(3, len(associations))

        type_id_tuples = [ (u['unit_type_id'], u['unit_id']) for u in associations]
        self.assertTrue(('type-1', 'unit-1') in type_id_tuples)
        self.assertTrue(('type-1', 'unit-2') in type_id_tuples)
        self.assertTrue(('type-2', 'unit-1') in type_id_tuples)

    def test_unit_associations_for_repo_no_associations(self):
        """
        Tests that retrieving the associations for a repo without any correctly
        returns an empty list.
        """

        # Test
        associations = self.manager.unit_associations_for_repo('empty')
